...
def dfs(curr, prev, finish, curr_depth, max_depth, is_start=False):
    global roads
    global distances
    ...
    for i in range(len(roads[curr])):
        if roads[curr][i] != 0:
            if distances[i] == -1:
                if dfs(i, curr, finish, curr_depth + 1, max_depth):
                    return True
    return False
...
for max_depth in range(15):
    distances = [-1] * len(cities)
    print(f"Depth: {max_depth}, Distance: ", end="")
    if dfs(city1, city1, city2, 0, max_depth, True):
